...
2. Verify Vercel production env vars per BUILD-SPEC M4 item 2 (rotated secrets from HT1, `NEXTAUTH_URL=https://byrachelpierce.com`, verified-domain `EMAIL_FROM`, real `GALLERY_EMAIL`) and that the latest production deploy is green — expected: all set; deploy green.
